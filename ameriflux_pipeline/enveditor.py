...
class EnvEditor():
    def __init__(self):
        ...
        self.LINE_PYFLUX_L1 = 93
        self.LINE_SAVE_ENV = 113
        ...
        # PyFluxPro L1 L2 related data
        self.PYFLUXPROPRO_L1_VARIABLE = " Variables for PyFluxPro AmeriFlux formatting"
        self.BROWSE_L1_INPUT = " PyFluxPro L1.txt file"
        self.DESC_L1_INPUT = " input L1.txt file for PyFluxPro"
        self.INFO_L1_INPUT = "The txt file for PyFluxPro's L1 process."
        self.BROWSE_L1_AMERIFLUX = " PyFluxPro L1.txt file with AmeriFlux formatting"
        self.DESC_L1_AMERIFLUX = " input L1.txt file with AmeriFlux specific formatting."
        self.INFO_L1_AMERIFLUX = "The txt file that will be generated by the process with AmeriFlux specific " \
                                 "formatting for PyFluxPro's L1 process.."
        ...
        self.INFO_L1_AMERIFLUX_MAINSTEM_KEY = "The excel file that contains the information about converting " \
                                              "variable names for AmeriFlux formatting."
        self.BROWSE_L1_OUTPUT = " L1 output nc file name"
        self.DESC_L1_OUTPUT = " output NC file name by AmeriFlux L1 process."
        self.INFO_L1_OUTPUT = "The output NC file that will be generated by the AmeriFlux formatted L1 process."
    ...
    def run(self):
        ...
        #############################################################
        # create pyfluxpro L1 main title
        ...
        # create L1 input
        label_l1_input = tk.Label(master=second_frame, text=self.BROWSE_L1_INPUT,
                                               font=self.BOLD_FONT).grid(sticky="w", row=i+4, column=0)
        info_l1_input  = tk.Button(second_frame, text=self.INFO_TITLE, font=self.MAIN_FONT,
                                               command=self.on_click_l1_input ). \
            grid(sticky="w", row=i+4, column=1)
        button_browse_l1_input  = \
            tk.Button(master=second_frame, text="Browse", font=self.MAIN_FONT,
                      command=self.browse_l1_input ).grid(sticky="w", row=i+4, column=2)
        desc_l1_input  = tk.Label(second_frame, text=self.DESC_L1_INPUT, font=self.DESC_FONT) \
            .grid(sticky="w", row=i+5, column=0, columnspan=3)
        self.path_l1_input  = tk.Label(master=second_frame, text=self.L1_INPUT,
                                                   font=self.MAIN_FONT)
        self.path_l1_input .grid(sticky="w", row=i+6, column=0, columnspan=3)
        label_separation = tk.Label(master=second_frame, text=self.SEPARATION_LABEL_SUB). \
            grid(sticky="w", row=i+7, column=0, columnspan=3)
        ...
        button_browse_l1_ameriflux = \
            tk.Button(master=second_frame, text="Browse", font=self.MAIN_FONT,
                      command=self.browse_l1_ameriflux).grid(sticky="w", row=i+8, column=2)
        ...
        self.path_l1_ameriflux = tk.Label(master=second_frame, text=self.L1_AMERIFLUX,
                                                font=self.MAIN_FONT)
        self.path_l1_ameriflux.grid(sticky="w", row=i+10, column=0, columnspan=3)
        label_separation = tk.Label(master=second_frame, text=self.SEPARATION_LABEL_SUB). \
            grid(sticky="w", row=i+11, column=0, columnspan=3)
        ...
        button_browse_l1_mainstem_key = \
            tk.Button(master=second_frame, text="Browse", font=self.MAIN_FONT,
                      command=self.browse_l1_mainstem_key).grid(sticky="w", row=i+12, column=2)
        ...
        self.path_l1_mainstem_key = tk.Label(master=second_frame, text=self.L1_AMERIFLUX_MAINSTEM_KEY,
                                                   font=self.MAIN_FONT)
        self.path_l1_mainstem_key.grid(sticky="w", row=i+14, column=0, columnspan=3)
        label_separation = tk.Label(master=second_frame,
                                    text=self.SEPARATION_LABEL_SUB).grid(sticky="w", row=i+15, column=0, columnspan=3)

        # create pyfluxpro L1 output ameriflux"
        label_l1_output = tk.Label(master=second_frame, text=self.BROWSE_L1_OUTPUT,
                                                   font=self.BOLD_FONT).grid(sticky="w", row=i+16, column=0)
        info_l1_output = tk.Button(second_frame, text=self.INFO_TITLE, font=self.MAIN_FONT,
                                                   command=self.on_click_l1_output). \
            grid(sticky="w", row=i+16, column=1)
        button_browse_l1_output = \
            tk.Button(master=second_frame, text="Browse", font=self.MAIN_FONT,
                      command=self.browse_l1_output).grid(sticky="w", row=i+16, column=2)
        desc_l1_output = tk.Label(second_frame, text=self.DESC_L1_OUTPUT,
                                                  font=self.DESC_FONT). \
            grid(sticky="w", row=i+17, column=0, columnspan=3)
        self.path_l1_output = tk.Label(master=second_frame, text=self.L1_OUTPUT,
                                                       font=self.MAIN_FONT)
        self.path_l1_output.grid(sticky="w", row=i+18, column=0, columnspan=3)
        label_separation = tk.Label(master=second_frame,
                                    text=self.SEPARATION_LABEL_SUB).grid(sticky="w", row=i+19, column=0, columnspan=3)
    ...
    def browse_l1_input(self):
        ...
        if filepath == "":
            ...
            filepath = filedialog.askopenfilename(initialdir=filepath,
                                                  title="select a file", filetypes=[("txt files", "*.txt")])
        self.path_l1_input.config(text=filepath)
        self.L1_INPUT = filepath
    ...
    def browse_l1_output(self):
        ...
        self.path_l1_output.config(text=filepath)
        self.L1_OUTPUT = filepath
    ...
    def save_env(self):
        ...
        pyfluxpro_l1_input_line = "L1_INPUT=" + self.L1_INPUT
        ...
        pyfluxpro_l1_output_line = "L1_OUTPUT=" + self.L1_OUTPUT
        ...
        lines = [
            ...
            "",
            pyfluxpro_l1_line, pyfluxpro_l1_input_line, pyfluxpro_l1_ameriflux_line,
            pyfluxpro_l1_ameriflux_mainstem_key_line, pyfluxpro_l1_output_line
        ]
    ...
    def on_click_l1_input(self):
        tk.messagebox.showinfo("Info", self.INFO_L1_INPUT)
    ...
    def on_click_l1_output(self):
        tk.messagebox.showinfo("Info", self.INFO_L1_OUTPUT)
