...
## Overview
- [pre_pyfluxpro](https://github.com/ncsa/ameriflux-pipeline/blob/develop/ameriflux_pipeline/pre_pyfluxpro.py) module does all the processing till the generation on PyFluxPro L1 and L2 control files.
...
- This module creates :
  - master meteorological data (see [mastermetprocessor](https://github.com/ncsa/ameriflux-pipeline/blob/develop/docs/master_met/mastermetprocessor.md) mentioned in [step#8](https://github.com/ncsa/ameriflux-pipeline/blob/develop/docs/prepyfluxpro.md#8)), 
  - master meteorological data formatted for eddypro (see [eddyproformat](https://github.com/ncsa/ameriflux-pipeline/blob/develop/docs/eddypro/eddyproformat.md) mentioned in [step#8](https://github.com/ncsa/ameriflux-pipeline/blob/develop/docs/prepyfluxpro.md#8)), 
  - eddypro full output (see [runeddypro](https://github.com/ncsa/ameriflux-pipeline/blob/develop/docs/eddypro/runeddypro.md) mentioned in [step#9](https://github.com/ncsa/ameriflux-pipeline/blob/develop/docs/prepyfluxpro.md#9)), 
  - pyfluxpro input excel sheet (see [pyfluxproformat](https://github.com/ncsa/ameriflux-pipeline/blob/develop/docs/pyfluxpro/pyfluxproformat.md) mentioned in [step#11](https://github.com/ncsa/ameriflux-pipeline/blob/develop/docs/prepyfluxpro.md#11)), 
  - pyfluxpro input excel sheet formatted for Ameriflux (see [amerifluxformat](https://github.com/ncsa/ameriflux-pipeline/blob/develop/docs/pyfluxpro/amerifluxformat.md) mentioned in [step#13](https://github.com/ncsa/ameriflux-pipeline/blob/develop/docs/prepyfluxpro.md#13)), 
  - L1 and L2 control files formatted for EPL-type processing (see [l1format](https://github.com/ncsa/ameriflux-pipeline/blob/develop/docs/pyfluxpro/l1format.md) and [l2format](https://github.com/ncsa/ameriflux-pipeline/blob/develop/docs/pyfluxpro/l2format.md) mentioned in [step#14](https://github.com/ncsa/ameriflux-pipeline/blob/develop/docs/prepyfluxpro.md#14) and [step#15](https://github.com/ncsa/ameriflux-pipeline/blob/develop/docs/prepyfluxpro.md#15)),
  - log file pre_pyfluxpro.log, and log file for each eddypro run in eddypro output folder.
- There is also an option to run each process separately. This can be achieved using the [pipeline](https://github.com/ncsa/ameriflux-pipeline/blob/develop/docs/pipeline.md) GUI.
- The Pre-Pyfluxpro module can be used with a GUI or command line.
...
### 3
- PyfluxPro does not recognize some Ameriflux variable names. PyfluxPro V3.3.2  throws an error when it encounters these names.
- These variables are called “Erroring Variables” in the pipeline and are included in a separate excel input file given in env variable L1_AMERIFLUX_ERRORING_VARIABLES_KEY
- A confirmation on whether this step is required or not is checked.
- If the env variable AMERIFLUX_VARIABLE_USER_CONFIRMATION is "Y", the user confirms to use Ameriflux variable names. The unrecognized ameriflux variable names are left alone and PyFluxPro V3.3.2 may not handle them correctly; an updated PyFluxPro version may be able to handle them in the future.
- If the env variable AMERIFLUX_VARIABLE_USER_CONFIRMATION is "N", the user confirms to use Pyfluxpro variable names (disagrees to use Ameriflux names). The Pyfluxpro variable names are left alone and should be recognizable to PyFluxPro software.
- If the AMERIFLUX_VARIABLE_USER_CONFIRMATION is "N", an erroring variable key is need to check which all variables are to remain as Pyfluxpro-friendly names.
- These variables are renamed to ameriflux standards in [post_pyfluxpro](https://github.com/ncsa/ameriflux-pipeline/blob/develop/ameriflux_pipeline/post_pyfluxpro.py) module.
- Any variables that are not in the Erroring variable key will automatically be renamed to Ameriflux labels using Ameriflux-Mainstem-Key and Soils key.
...
### 8
- First, the pre_processing() method calls the [eddypro_preprocessing()](https://github.com/ncsa/ameriflux-pipeline/blob/develop/ameriflux_pipeline/pre_pyfluxpro.py#L78) method to create the master meteorological data and format it for eddypro input.
- eddypro_preprocessing() method calls [mastermetprocessor](https://github.com/ncsa/ameriflux-pipeline/blob/develop/docs/master_met/mastermetprocessor.md) module to create the master meteorological data 
...
- Secondly, the eddypro_processing() method calls [eddyproformat](https://github.com/ncsa/ameriflux-pipeline/blob/develop/docs/eddypro/eddyproformat.md) module to format the master meteorological for eddypro input and stores the site soil moisture and temperature variables.

### 9
- Next step is to run eddypro software in a headless manner.
- Validation is done to check if the user chosen path in .env setting EDDYPRO_OUTPUT_PATH to make sure it is an empty directory.
- If the directory is not empty, its existing contents are moved to another directory named "<directoryname>_run_result_<timestamp>".
- This is done because the new eddypro run would overwrite the contents of a previous run in the eddypro output directory.
...
### 10
- At the conclusion of the runeddypro module, the eddypro 'full_output' file is checked in the EDDYPRO_OUTPUT_PATH to ensure that eddypro output was produced.
...
### 13
- On successful creation of pyfluxpro input sheet, the pre_pyfluxpro module calls [pyfluxpro_ameriflux_processing()](https://github.com/ncsa/ameriflux-pipeline/blob/develop/ameriflux_pipeline/pre_pyfluxpro.py#L216) method to create pyfluxpro input sheet formatted for Ameriflux processing.
...
- This method takes in inputs : 
  - PYFLUXPRO_INPUT_AMERIFLUX 
    - pyfluxpro input sheet formatted for Ameriflux (created in step#13)
  - L1_MAINSTEM_INPUT 
    - Pyfluxpro L1 control file used for mainstem processing. 
    - Variables present in this file are written to output L1 after validation checks. 
    - This file also provides variable attributes (e.g. units) and the details in the Global section.
  ...
  - L1_AMERIFLUX_MAINSTEM_KEY 
    - An excel file that the user can use to select which variables to write in the output L1 file.
    - Pairs original met and flux variable names with matching Ameriflux variable names, e.g. “SW_Dn_Avg” (original) with “SW_IN” (Ameriflux name for the same quantity).
    - Only those variables present in this file will be written to the output L1.
    - The content description of this file can be found in [config.md](https://github.com/ncsa/ameriflux-pipeline/blob/develop/docs/config.md).
  - file_meta_data_file
...
- This method takes in input :
  - ameriflux_mapping
    - Contains the variable mapping established by the in l1format module described in step #14 
    - The Ameriflux labels in this file will be the variable names present in output L1 and L2 control files.
  - L2_MAINSTEM_INPUT
